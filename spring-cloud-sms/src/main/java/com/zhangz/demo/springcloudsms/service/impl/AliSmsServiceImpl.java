...
public class AliSmsServiceImpl implements AliSmsService {
    ...
    public void sendSms(AliSms aliSms) throws Exception {
        ...
        // 1. 初始化客户端
        Client client = createClient(aliSms.getAccessKeyId(), aliSms.getAccessKeySecret());

        // 2. 创建请求&运行时配置对象
        // 创建请求对象并设置入参
        SendSmsRequest sendSmsRequest = new SendSmsRequest().setPhoneNumbers(aliSms.getPhoneNumbers()).setSignName(aliSms.getSignName()).setTemplateCode(aliSms.getTemplateCode())
            .setTemplateParam(JSON.toJSONString(aliSms.getTemplateParam()));
        // 使用默认运行时配置发起接口调用
        ...
        // 3.查询结果
        String[] phoneNums = StringUtils.split(aliSms.getPhoneNumbers(), ",");
        for (String phoneNum : phoneNums) {
            QuerySendDetailsRequest queryReq = new QuerySendDetailsRequest().setPhoneNumber(com.aliyun.teautil.Common.assertAsString(phoneNum)).setBizId(bizId)
                .setSendDate(DateUtils.format(new Date(), "yyyyMMdd")).setPageSize(10L).setCurrentPage(1L);
